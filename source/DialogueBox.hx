	...
	public function new(talkingRight:Bool = true, ?dialogueList:Array<String>)
	{
		...
		if(PlayState.isStoryMode){ //Added an extra check here to avoid music playing when in freeplay.
			switch (PlayState.SONG.song.toLowerCase())
			{
				...
				case 'censory-superload' | 'exterminate':
		...
		switch (PlayState.SONG.song.toLowerCase())
		{
			...
			case 'censory-superload' | 'exterminate': //Why are they seperated? ...Erm...  ¯\_(ツ)_/¯
				hasDialog = true;
				box.frames = Paths.getSparrowAtlas('speech_bubble_talking');
				box.animation.addByPrefix('normalOpen', 'Speech Bubble Normal Open', 24, false);
				box.animation.addByPrefix('normal', 'speech bubble normal', 24, false);
		...
		if (PlayState.SONG.song.toLowerCase()=='carefree' || PlayState.SONG.song.toLowerCase()=='careless' || PlayState.SONG.song.toLowerCase()=='censory-superload' || PlayState.SONG.song.toLowerCase()=='exterminate' || PlayState.SONG.song.toLowerCase()=='cessation')
		{
			...
			if(PlayState.SONG.song.toLowerCase()=='censory-superload'){ //Defining portraits for Censory-Superload
			...
			}else if(PlayState.SONG.song.toLowerCase()=='exterminate'){ //Defining portraits for Exterminate 
				portraitLeft = new FlxSprite(-20, 50);
				portraitLeft.frames = Paths.getSparrowAtlas('ui/roboPortait');
				portraitLeft.animation.addByPrefix('enter', 'robo_potrait', 24, false);
				portraitLeft.updateHitbox();
		...
		if(PlayState.SONG.song.toLowerCase()=='exterminate' || PlayState.SONG.song.toLowerCase()=='cessation'){
			box.x+=50;
			portraitLeft.y += 110;
			portraitLeft.x -= 400;
		...
		if (FlxG.keys.justPressed.ANY #if android || justTouched #end && dialogueStarted == true)
		{
			...
			if (dialogueList[1] == null && dialogueList[0] != null)
			{
				if (!isEnding)
				{
					...
					else if(PlayState.SONG.song.toLowerCase() == 'censory-superload' || PlayState.SONG.song.toLowerCase()=='exterminate' || PlayState.SONG.song.toLowerCase()=='carefree')
