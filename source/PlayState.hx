...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{				
			...
			case 'streetFinal' | 'streetCute' | 'street' :
				boyfriend.x += 40;
				boyfriend.y += 65;
				if(SONG.song.toLowerCase() == 'censory-superload' || SONG.song.toLowerCase() == 'extermination'){
					dad.x -= 70;
					dad.y += 66;
					if(!Main.qtOptimisation){
						boyfriend404.x += 40;
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		else if (SONG.song.toLowerCase() == 'censory-superload' && !cutsceneSkip)
		{
			add(horrorStage);
		}
		...
		new FlxTimer().start(0.3, function(tmr:FlxTimer)
		{
			...
			else
			{
				if (dialogueBox != null)
				{
					...
					if (SONG.song.toLowerCase() == 'censory-superload' && !cutsceneSkip)
					{
						//Background old
	...
	function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			if(!Main.qtOptimisation && (SONG.song.toLowerCase()=='censory-superload' || SONG.song.toLowerCase() == 'extermination')){
				dad404.dance();
				gf404.dance();
	...
	override public function update(elapsed:Float)
	{
		...
		if (curSong.toLowerCase() == 'censory-superload'){
			...
			if (!dad.animation.curAnim.name.startsWith("sing") && curStep % 2 == 0)
				{
					dad.dance();
				}
			if(curStep == 128){
	...
	{
		if(!(SONG.song.toLowerCase() == "extermination" || SONG.song.toLowerCase() == "tutorial" || SONG.song.toLowerCase() == "censory-superload")){
			trace("Sawblade Attack Error, cannot use Extermination functions outside Extermination, Tutorial or Censory-Superload.");
		}
		trace("HE ATACC!");
		if(state){
	...
	{
		if(!(SONG.song.toLowerCase() == "extermination" || SONG.song.toLowerCase() == "tutorial" || SONG.song.toLowerCase() == "censory-superload")){
			trace("Sawblade Alert Error, cannot use Extermination functions outside Extermination, Tutorial or Censory-Superload.");
		}
		trace("DANGER!");
	...
	private function keyShit():Void
	{
		...
		//Dodge code only works on termination and Tutorial -Haz -and Censory-Superload (Censory-Overload) -DrkFon376
		if(SONG.song.toLowerCase() == "extermination"){
		...
		if(SONG.song.toLowerCase() == 'tutorial' || SONG.song.toLowerCase()=='censory-superload'){
			//Dodge code, yes it's bad but oh well. -Haz
			//var dodgeButton = controls.ACCEPT; //I have no idea how to add custom controls so fuck it. -Haz
			...
			if(_vpad.buttonA.justPressed && !bfDodging && bfCanDodge){
				...
				new FlxTimer().start(bfDodgeTiming, function(tmr:FlxTimer) 	//COMMENT THIS IF YOU WANT TO USE DOUBLE SAW VARIATIONS!
				{
					...
					{
						bfCanDodge=true;
						trace('DODGE RECHARGED!');//I've separated the dodge code from Censory-Superload and Tutorial so that the Bf animation lasts as long as it needs to last -DrkFon376
					});
				});
			}
		}
